...
if __name__ == "__main__":
    ...
    # [ADDED]: Command line argument to toggle INT8 simulation
    parser.add_argument("--simulate-int8", action="store_true", help="Enable simulated INT8 truncation error in Gram NS.")
    ...
    our_method = GramNewtonSchulz(
        ns_use_kernels=not args.pure_pytorch, 
        ns_coefficients=coeffs_steps, 
        gram_newton_schulz_reset_iterations=args.restarts,
        simulate_int8=args.simulate_int8,  # <-- [ADDED]: Pass the toggle
        compile_kwargs=None      # <-- [MODIFIED]: Avoid triton
    )
    df = run_comparison(our_method, StandardNewtonSchulz(ns_use_kernels=not args.pure_pytorch, ns_coefficients=coeffs_steps, compile_kwargs=None), matrix_iter, iterlen=num)
    test_set_name = os.path.basename(os.path.normpath(args.test_set))

    # [ADDED]: Append a tag to the output files so they don't overwrite standard benchmarks
    if args.simulate_int8:
        test_set_name += "_INT8_SIMULATED"
